SET LINES 250 PAGES 5000 TIMING ON TRIM ON ECHO ON
SET MARKUP HTML ON
...
--Collect DB Info by AzizPW version 5.5
REM Collect DB Info by AzizPW version 5.5
ALTER SESSION SET NLS_DATE_FORMAT='DD-Mon-RR HH24:MI:SS';

--Document information
COL USER FOR A15
SELECT USER, 'OracleDB Info by AzizPW ver. 5.5' SCRIPT, SYSDATE COLLECT_DATE FROM DUAL;
...
COL SCHEMA FOR A25;
COL STATUS FOR A10;
SELECT COMP_ID,COMP_NAME,VERSION,STATUS,MODIFIED,SCHEMA FROM DBA_REGISTRY ORDER BY 1;
COL CNAME FOR A45;
COL VERSION FOR A10;
SELECT CID,CNAME,VERSION,TO_CHAR(STATUS)STATUS,FLAGS,TO_CHAR(MODIFIED,'DD-Mon-RR HH24:MI:SS')MODIFIED FROM REGISTRY$ ORDER BY 1,2;
...
COL PATH FOR A50;
COL NAME FOR A25;
SELECT MOUNT_STATUS,HEADER_STATUS,STATE,ROUND(TOTAL_MB/1024,2)TOTAL_GB,ROUND(FREE_MB/1024,2)FREE_GB,NAME,PATH FROM V$ASM_DISK ORDER BY 6,7;
SELECT NAME,TYPE,ROUND(TOTAL_MB/1024,2)TOTAL_GB,ROUND(FREE_MB/1024,2)FREE_GB,ROUND(USABLE_FILE_MB/1024,2)USABLE_FILE_GB,STATE,SECTOR_SIZE,BLOCK_SIZE FROM V$ASM_DISKGROUP ORDER BY 1;
COL ASMDISK FOR A15;
COL DISKGROUP FOR A25;
SELECT DG.NAME DISKGROUP,DG.TYPE,D.NAME ASMDISK,D.PATH,ROUND(D.TOTAL_MB/1024,2)TOTAL_GB,ROUND(D.FREE_MB/1024,2)FREE_GB,D.VOTING_FILE,DG.SECTOR_SIZE,DG.BLOCK_SIZE FROM V$ASM_DISKGROUP DG JOIN V$ASM_DISK D ON DG.GROUP_NUMBER=D.GROUP_NUMBER ORDER BY 3,4;
...
--Database size
SELECT /*+ PARALLEL(4)*/ROUND((SUM(BYTES))/1024/1024/1024, 2)DBSIZE_GB,ROUND((SUM(BYTES))/1024/1024/1024/1024, 2)DBSIZE_TB FROM V$DATAFILE;

--Total database size
SELECT /*+ PARALLEL(4)*/ 'Datafiles' "COMPONENTS", ROUND((SUM(BYTES))/1024/1024/1024, 2)DATAFILES_GB FROM V$DATAFILE UNION ALL
SELECT /*+ PARALLEL(4)*/ 'Tempfiles' "COMPONENTS", ROUND((SUM(BYTES))/1024/1024/1024, 2)TEMPFILES_GB FROM V$TEMPFILE UNION ALL
...
SELECT USERNAME, PROFILE, ACCOUNT_STATUS, LOCK_DATE, EXPIRY_DATE, DEFAULT_TABLESPACE, TEMPORARY_TABLESPACE TEMP_TBS FROM DBA_USERS ORDER BY 1;
...
COL INPUT_BYTES_DISPLAY FOR A19;
COL OUTPUT_BYTES_DISPLAY FOR A20;
COL OUTPUT_BYTES_PER_SEC_DISPLAY FOR A28;
