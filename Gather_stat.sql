...
SELECT OWNER,TABLE_NAME,PARTITION_NAME,OBJECT_TYPE,LAST_ANALYZED,STALE_STATS FROM DBA_TAB_STATISTICS WHERE OWNER NOT IN ('SYS','SYSTEM','MDSYS','DBSNMP') AND (STALE_STATS='YES' OR STALE_STATS IS NULL) ORDER BY LAST_ANALYZED DESC;
...
SET TIME ON LINES 150 PAGES 5000;
SELECT 'EXEC DBMS_STATS.GATHER_SCHEMA_STATS('''||USERNAME||''', ESTIMATE_PERCENT => DBMS_STATS.AUTO_SAMPLE_SIZE, CASCADE => TRUE, METHOD_OPT=>''FOR ALL COLUMNS SIZE AUTO'', DEGREE=>4);'
