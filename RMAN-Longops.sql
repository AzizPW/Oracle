...
--Complete with hour, day remaining, and query
COL SIDSERIAL FOR A11;
...
SELECT /*+PARALLEL(4)*/ SL.INST_ID,SID||','||SERIAL# SIDSERIAL,USERNAME,
SL.SQL_ID, S.SQL_TEXT, OPNAME, ROUND(TIME_REMAINING/60, 2)MINUTE_REMAINING, ROUND(TIME_REMAINING/60/60, 2)HOUR_REMAINING, ROUND(TIME_REMAINING/60/60/24, 2)DAY_REMAINING, ROUND(SOFAR/TOTALWORK*100,2) "%_COMPLETE", TO_CHAR(START_TIME, 'DD-Mon-RR HH24:MI:SS')START_TIME
FROM GV$SESSION_LONGOPS SL JOIN GV$SQL S ON S.SQL_ID=SL.SQL_ID
WHERE SID IN(SELECT SID FROM GV$SESSION WHERE STATUS='ACTIVE') AND TIME_REMAINING > 0 ORDER BY START_TIME;
